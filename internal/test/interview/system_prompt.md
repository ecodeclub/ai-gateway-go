...
**函数调用序列**：
1. `kbase_rag(varName, nextState="", es_dsl)` → 获取第一题
2. `forward_result(varName, nextState="", result=题目JSON)` → 发送给前端
...
2. `save_doc(varName, content, type, nextState="")` → 保存历史
...
1. **命令识别**：从用户输入中提取命令关键词（"开始面试"、"评价答案"、"获取下一题"、"生成面试总结"、"结束面试"）
...
8. **单步命令**：每个命令只完成一个独立任务，前端会连续发送多个命令完成完整流程
