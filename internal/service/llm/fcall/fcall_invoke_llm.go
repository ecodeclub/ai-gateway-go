...
func NewInvokeLLMFuncCall(svc *service.ChatService,
	renderClient *template.DefaultRender,
	repo *repository.InvocationConfigRepo) *InvokeLLMFuncCall {
...
func (c *InvokeLLMFuncCall) Call(ctx *Context, req Request) (Response, error) {
	...
	// 调用llm接口
	resp, err := c.svc.Chat(ctx, uuid.New().String(), []domain.Message{
		{
			Role:    domain.USER,
			Content: prompt,
		},
	...
	ctx.SetAttachment(c.Name(), string(llmResp))
	return Response{}, nil
}
